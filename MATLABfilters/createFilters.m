...
%%
% designs for different sampling frequencies
% write the filter file header
...
[yk1]=filter(b,a,xk);
if play==1
    disp('Yule-Walker treble boost filter')
...
% test filter
if play==1
    disp('echo filter (feedforeward only)')
    [yk1]=filter(b,a,xk);
